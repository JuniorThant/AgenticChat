...
export default function InputContainer({ messages, setMessages }: MessageProps) {
  ...
  const handleSend = async () => {
    ...
    if (file) newMessages.push({ text: "", isUser: true, imageUrl: URL.createObjectURL(file) });
  ...
  return (
    <div className="border-t px-4 py-3 flex items-center gap-2">
      ...
      {/* Show Image Preview */}
      {file && (
        <button onClick={() => setShowModal((prev) => !prev)} className="hover:text-blue-500">
          <FaArrowUp />
        </button>
      )}

      {/* Photo Upload */}
      <label htmlFor="file-upload" className="cursor-pointer bg-gray-200 px-3 py-2 rounded-md">
        <FaCameraRetro />
      </label>

      {/* Thinking Icon */}
      <button onClick={()=>setIsThinking((prev)=>!prev)} className={`cursor-pointer bg-gray-200 px-3 py-2 rounded-md ${isThinking ? "text-blue-500" : ""}`}>
        <FaQuestion />
      </button>
      ...
      {isThinking &&<div className="absolute bottom-20 right-6 bg-white border border-black shadow-md p-2 z-10">Thinking Mode is on</div>}
      ...
      {showModal && file && (
        <div className="absolute bottom-20 right-4 bg-white border shadow-md rounded-md p-2 z-10">
          ...
          <img src={URL.createObjectURL(file)} alt="image-preview" className="w-40 h-40 object-cover rounded-md" />
        </div>
      )}
    </div>
  );
